...
export default function BatchHistoryTable({ movements }: { movements: HistoryMovement[] }) {
  const [expandedBatches, setExpandedBatches] = useState<Set<number>>(new Set());
  const [modalBatch, setModalBatch] = useState<BatchGroup | null>(null);

  // Group inbound movements by batch_id; individual movements stay separate
  ...
  // Merge batch groups + individual rows, sorted by date descending
  type DisplayRow =
        ...
        <table className="w-full text-sm min-w-[600px]">
          ...
          <tbody>
            {displayRows.map(row => {
              ...
              return (
                <tr key={`mov-${m.id}`} className="border-t border-bone hover:bg-cream/50">
